...
void *func()
{
	while(1)
	{
		sleep(30);
		time(&timep);
		fprintf(pfd, "%snetclient1 is working.\n", asctime(gmtime(&timep)));
		fflush(pfd);
	}
	pthread_exit(NULL);
}
...
int main(int argc, char *argv[])
{
	...
	pfd = fopen("/home/pi/log/netclient1.log", "a");
	setbuf(pfd, NULL);
	if(pfd == NULL)
	{
		perror("fopen netclient1.log fail");
		return 0;
	}

	time(&timep);
	fprintf(pfd, "%snetclint1 start work.\n", asctime(gmtime(&timep)));
	fflush(pfd);
	...
	printf("connect ok\n");
#endif
	struct timeval timeout = {5,0};//5s超时
